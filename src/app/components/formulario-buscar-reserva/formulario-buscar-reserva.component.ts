...
import {destionosOpcoes} from 'src/app/buscar-hoteis/store/buscar-hoteis.selectors';
...
export class FormularioBuscarReservaComponent implements OnDestroy {
  ...
  destionosOpcoes$: Observable<KeyValuePair[]>;
  ...
  constructor(private formBuilder: FormBuilder, private store: Store<fromApp.AppState>) {
    ...
    this.destionosOpcoes$ = this.store.select(destionosOpcoes);
